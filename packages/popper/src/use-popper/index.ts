import { computed, ref, reactive, watch, CSSProperties, unref } from 'vue'
...
import {
  ...
  isArray,
  isString,
} from '@hkust-ui/utils/util'
import PopupManager from '@hkust-ui/utils/popup-manager'
...
export default function(
  ...
  // hide the popper and destory if props.disabled = true
  const close = () => {
    _hide()
    if (props.disabled) {
      doDestroy(true)
  ...
  // hide the popper when mouse leave, except trigger is click or focus
  function onPopperMouseLeave() {
    const { trigger } = props
    // if trigger mode is click or focus, if should'n hide when the mouse leave
    const shouldPrevent =
