...
const icons = icon.map(item => 'hkust-icon-' + item).join('/')
...
function reComponentName(title) {
  return 'hkust-' + title.replace(/\B([A-Z])/g, '-$1').replace(/[ ]+/g, '-').toLowerCase()
}

function reDocUrl(fileName, header) {
  const docs = 'https://gitee.com/x_c_huang/hkust-ui.git/#/en-US/component/'
